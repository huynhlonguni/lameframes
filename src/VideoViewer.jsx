...
import GetVideoID from "./video_lookup";
import Thumbnail from "./Thumbnail";
import { GetKeyframeInfo, GetNearestKeyframes } from "./keyframe_lookup";
...
const VideoViewer = ({viewer, onClose, onSubmit, onLocalSearch}) => {
			...
			<div className="bg-white rounded-lg h-5/6 w-5/6 absolute" onClick={(e) => {e.stopPropagation();}}>
				<div className="flex flex-col place-items-center p-4 h-full">
					...
					<div className="grid grid-cols-7 w-full px-4 gap-4">
						...
						<div className="grid grid-rows-2 gap-2">
							<div onClick={onLocalSearch} className="p-4 w-full h-full bg-slate-200 hover:bg-slate-300 cursor-pointer rounded-lg">
								<SearchTypeRenderer className="font-bold" type={SearchType.LOCAL_SEARCH} />
							</div>
							<div onClick={onSubmit} className="p-4 w-full h-full text-white bg-cyan-600 hover:bg-cyan-700 cursor-pointer rounded-lg">
								<div className="flex place-items-center gap-2">
